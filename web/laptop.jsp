			...
			<div class="body">
				<ul>
				                <% int sn,en;
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        try
                                                        {
                                                            ...
                                                            if(request.getParameter("cat")==null||request.getParameter("cat").equals("null"))
                                                            { ps = con.prepareStatement("select * from (select a.*,rownum rnum from(select laptop_system_info.*,brand.brand_name,brand.brandimage from  laptop_System_info,brand where brand.brandid = laptop_system_info.brandid order by model_no) a where rownum<=?) where rnum>=?");
                                                              ps.setInt(1,en);
                                                              ps.setInt(2,sn);
                                                            }
                                                            else
                                                            {
                                                                  ps = con.prepareStatement("select * from (select a.*,rownum rnum from(select laptop_system_info.*,brand.brand_name,brand.brandimage from  laptop_System_info,brand where brand.brandid = laptop_system_info.brandid and category=? order by model_no) a where rownum<=?) where rnum>=?");
                        ...
                        out.println("</tr>");*/
							...
							out.println("<a href=views.jsp?modelno="+mn.replace(" ","%20")+"&ptype=laptop>");
                                                %>
						...
						<div>
							...
							<h3><%=mn%></h3>
                                                        ...
                                                        <p> Brand            :<%=rs1.getString("brand_name")%> <br>
                                                            Operating System : <%=rs1.getString("operating_system")%><br>
                                                            Processor : <%=rs1.getString("processor")%><br>
                                                            RAM : <%=rs1.getString("ram")%><br>
                                                            warranty : <%=rs1.getString("warrenty")%> Years<br>
                                                            Screen Size : <%=rs1.getString("screensize")%> inch<br>
                                        ...
                                        <% 
                                            ...
                                            if(request.getParameter("cat")==null||request.getParameter("cat").equals("null"))
                                                ...
                                                rs2=st2.executeQuery("select count(*) from laptop_System_info ");
                                            else
                                            rs2=st2.executeQuery("select count(*) from laptop_System_info where category='"+request.getParameter("cat")+"'");
                                            ...
                                            {
                                                out.println("<li><a href=laptop.jsp?sn="+k+"&en="+(2+k)+"&cat="+request.getParameter("cat")+">"+i+"</a></li>");
                                                k=k+3;
                                            }
                                            }
                                            }
